...
class Model(nn.Module):
    def __init__(self, config, load_emb=False, path=None, bias=True, total_tokens=63, depth=5, top_k=8, threshold=1.0):
        ...
        # SVIP parameters for dynamic draft length
        self.svip_enabled = getattr(config, 'svip_enabled', False)
        self.svip_entropy_threshold = getattr(config, 'svip_entropy_threshold', 0.3)
        self.svip_max_draft_tokens = getattr(config, 'svip_max_draft_tokens', 20)
        
        # Override total_tokens if SVIP is enabled
        if self.svip_enabled:
            self.total_tokens = min(self.total_tokens, self.svip_max_draft_tokens - 1)
    ...
    def topK_genrate(self, hidden_states, input_ids, head, logits_processor):
        ...
        # 4
        actual_depth = 0  # Track actual depth for SVIP
        for i in range(depth):
            ...
            # SVIP: Check entropy and decide whether to continue drafting
            if self.svip_enabled:
                # Calculate entropy: H = -sum(p * log(p))
                probs = torch.exp(last_p)  # Convert log probs to probs
                entropy = -torch.sum(probs * last_p, dim=-1)  # Shape: [top_k]
                avg_entropy = entropy.mean()  # Average entropy across top_k candidates
                
                # Check stopping condition: sqrt(H) > threshold
                if torch.sqrt(avg_entropy) > self.svip_entropy_threshold:
                    # Stop drafting early
                    break
        ...
        del mask_index_list
        return draft_tokens, retrieve_indices, tree_mask, tree_position_ids, retrieve_probs
